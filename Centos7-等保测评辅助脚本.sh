...
echo "当前系统版本：$(([ -f /etc/centos-release ] && echo `cat /etc/centos-release`) || ([ -f /etc/redhat-release ] && echo `cat /etc/redhat-release`) || echo -e "\033[31m[-] 没有找到系统版本，您的系统可能不是Centos7，执行可能出现一些报错!\033[0m")"
echo "当前系统内核版本：`uname -a`" 
echo "当前系统时间：$(date +"%Y-%m-%d %H:%M:%S")"
echo "当前系统CPU使用情况：$(top -n 1 | \grep "%Cpu")"
echo "当前系统内存使用情况：$(top -n 1 | \grep "KiB Mem")"
...
cat /etc/audit/audit.rules >> ./res_config.log
